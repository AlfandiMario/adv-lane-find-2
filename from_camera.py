...
if __name__ == '__main__':
    ...
    writer= cv2.VideoWriter(judul, cv2.VideoWriter_fourcc(*'mp4v'), 20, (width,height))
    ...
    while True:        
        ...
        for i in range(0,60):
            _, image = cam.read()
            # Buat File Gambar
            img_name = "bahanvid/frame_{}.jpg".format(counter)
            img = cv2.imwrite(img_name, image)
            # Masukan ke Array
            img_array.append(img)
            # Buat File Video
            frame = cv2.imread("bahanvid/frame_{}.jpg".format(counter))
            writer.write(frame)
            counter+=1
        ...
        #     frame = cv2.imread("bahanvid/frame_{}.jpg".format(a))
        #     writer.write(frame)
        
        writer.release()
        print("Output Sudah Keluar")

        # clip = VideoFileClip(judul).fl_image(process_pipeline)
        # clip = VideoFileClip(judul)
        # clip.write_videofile('from_cam_out.mp4', audio=False)
